...
function sessionId(ref: string): string {
  return `claude-${createHash('sha256').update(ref).digest('hex').slice(0, 24)}`
}
...
class ClaudeCodeSource implements SessionSource {
  readonly id = 'claude-code'

  async discover(homeDir: string): Promise<SessionRef[]> {
    const files = await discoverJsonl(join(homeDir, '.claude', 'projects'))
  ...
  async parse(ref: SessionRef): Promise<ParsedSession> {
    ...
    for (const [index, line] of lines.entries()) {
      ...
      if (type === 'user') {
        ...
        if (!firstUserText) {
          firstUserText = messageText(message?.content)
          firstUserAt = at
        }
...
export const claudeCodeSource: SessionSource = new ClaudeCodeSource()
